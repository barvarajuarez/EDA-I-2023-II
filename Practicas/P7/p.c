...
struct Node* searchNode(struct Node* last, int value) {
  // si la lista está vacía, se devuelve un puntero nulo
  if (last == NULL) return NULL;

  struct Node* current = last->next;

  // se recorre la lista hasta encontrar el valor o hasta volver al principio
  while (current != last) {
    if (current->data == value) return current;
    current = current->next;
    printf("se encuentra en la estructura");
  }

  // se comprueba el último nodo, ya que no se procesa en el bucle
  if (current->data == value) return current;
  printf("se encuentra en la estructura");

  // si no se encuentra el valor, se devuelve un puntero nulo
  printf("no esta en la estructura");
  return NULL;
}

int main() {
  int current;
  printf ("que ingrese el elemento que desea buscar: ");
  scanf("%d", &current);
